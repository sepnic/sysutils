...
public:
  ...
  static std::string Join(const ContainerT& things, SeparatorT separator) {
    if (things.empty()) {
      return "";
    }
...
extern template std::string StringUtils::Join(const std::vector<std::string>&, char);
extern template std::string StringUtils::Join(const std::vector<const char*>&, char);
