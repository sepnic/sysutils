/*
 * Copyright (C) 2018-2021 luoyun <sysu.zqlong@gmail.com>
 *
...
static unsigned char* InitDecodeTable() {
    // double buffer is used here for thread safe.
    static unsigned char DecodeTableBuff[256];
    memset(DecodeTableBuff, 0x80, sizeof(DecodeTableBuff));
    for (size_t k = 0; k < sizeof(EncodeTable); ++k) {
        DecodeTableBuff[(size_t)EncodeTable[k]] = k;
    }
    // to mark those valid characters in encoded string, but not in these
    // 64 bases list.
    DecodeTableBuff[(size_t)'\r'] = DecodeTableBuff[(size_t)'\n'] = 0x4F;
    DecodeTableBuff[(size_t)'='] = 0x40;
    return &DecodeTableBuff[0];
}
static const unsigned char* DecodeTable = InitDecodeTable();
...
static int GetNext4EncodedCharacters(const unsigned char*& p,
...
bool Base64Encode(const std::string& input, std::string* output)
{
    ...
    output->resize(Base64EncodeExpectedLength(input.size()));
...
bool Base64Encode(const std::string& input, char* output, size_t* len)
{
    ...
    char* buff = output;
    if (*len < Base64EncodeExpectedLength(input.size())) {
        return false;
    }
...
{
    if (output == NULL) return false;
    output->resize(Base64DecodeExpectedLength(input.size()));
...
{
    if (output == NULL || len == NULL) return false;

    char* buff = output;
    if (*len < Base64DecodeExpectedLength(input.size())) {
        return false;
    }
    if (input.empty()) {
        *len = buff - output;
...
int main()
{
    static const std::string encode_str[7] = {
        "1",
        "12",
        "1234",
        "12345678",
    ...
    static const std::string decode_str[7] = {
        "MQ==",
        "MTI=",
        "MTIzNA==",
        "MTIzNDU2Nzg=",
    ...
    std::string result;
    for (int i = 0; i < 7; ++i) {
        Base64Encode(encode_str[i], &result);
        printf("-->Encode: input[%s] : output[%s]\n", encode_str[i].c_str(), result.c_str());
        Base64Decode(decode_str[i], &result);
        printf("<--Dncode: input[%s] : output[%s]\n\n", decode_str[i].c_str(), result.c_str());
    }
