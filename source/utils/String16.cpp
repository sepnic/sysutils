...
String16::String16(const char16_t* o)
{
    ...
    OS_ASSERT(buf, LOG_TAG, "Unable to allocate shared buffer");
    if (buf) {
        char16_t* str = (char16_t*)buf->data();
        strcpy16(str, o);
        mString = str;
...
bool String16::setTo(const String16& other, size_t len, size_t begin)
{
    ...
    if (&other == this) {
        OS_FATAL_IF(true, LOG_TAG, "Not implemented");
    }

    return setTo(other.string()+begin, len);
}

bool String16::setTo(const char16_t* other)
{
    return setTo(other, strlen16(other));
}

bool String16::setTo(const char16_t* other, size_t len)
{
...
bool String16::append(const String16& other)
{
...
bool String16::append(const char16_t* chrs, size_t otherLen)
{
    const size_t myLen = size();
    if (myLen == 0) {
        setTo(chrs, otherLen);
...
bool String16::insert(size_t pos, const char16_t* chrs)
{
    return insert(pos, chrs, strlen16(chrs));
}

bool String16::insert(size_t pos, const char16_t* chrs, size_t len)
{
    const size_t myLen = size();
    if (myLen == 0) {
...
bool String16::replaceAll(char16_t replaceThis, char16_t withThis)
{
    const size_t N = size();
    const char16_t* str = string();
    char16_t* edit = NULL;
...
bool String16::remove(size_t len, size_t begin)
{
    const size_t N = size();
    if (begin >= N) {
        SharedBuffer::bufferFromData(mString)->release();
